...
| **Admin portal** | Loopback web console (`hub admin`, `http://127.0.0.1:8788/admin/`) — same inventory APIs, Bearer token; **not** HTTP MCP |
...
ansible-flow-mcp hub serve                # alias for hub admin
...
| `ANSIBLE_FLOW_ADMIN_PORT` | Admin HTTP port (default `8788`) |
...
- Lab: host `http://127.0.0.1:8788/admin/` (compose publishes loopback).
